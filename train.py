...
def create_model(use_conv, system_constants=None):
    if use_conv:
    ...
    def create_layer(no_units, activation='gelu'):
        if use_conv:
    ...
    if use_conv:
        ...
        second_net = keras.layers.Conv3D(no_units, kernel_size=(3, 3, 3), activation='gelu', padding='same')(net)
        second_net = keras.layers.Conv3D(no_units, kernel_size=(3, 3, 3), activation='gelu', padding='same')(second_net)
...
def fine_tune_loss_fn(y_true, y_pred):
    ...
    # As estimated from real data
    std_dev = 0.08
    loss = -(-std_dev - (1.0 / 2.0) * (residual / std_dev) ** 2)

    loss = tf.reduce_sum(loss, -1)
    loss = tf.where(mask > 0, loss, tf.zeros_like(loss))
...
def prepare_dataset(real_data, model):
    ...
    normalised_data = tf.where(real_data[:, :, :, :, -1:] > 0,
                               tf.math.log(real_data[:, :, :, :, :-1] / real_data[:, :, :, :, 2:3]),
                               tf.zeros_like(real_data[:, :, :, :, :-1]))
    predicted_distribution, _ = model.predict(normalised_data)
...
def save_predictions(model, data, filename, use_first_op=True):
    ...
    data = tf.where(data[:, :, :, :, -1:] > 0, tf.math.log(data[:, :, :, :, :-1] / data[:, :, :, :, 2:3]),
                    tf.zeros_like(data[:, :, :, :, :-1]))
    predictions, predictions2 = model.predict(data)
...
if __name__ == '__main__':
    ...
    model.fit(synthetic_dataset, epochs=10, callbacks=[mc], validation_data=(valid_x, valid_y))
    ...
    full_optimiser = tf.keras.optimizers.Adam(learning_rate=1e-5)
    ...
    def predictions_loss(t, p):
        return kl_loss(t, p) + smoothness_loss(t, p)*10.0
    full_model.compile(full_optimiser,
                       loss={'predicted_images': fine_tune_loss_fn,
                             'predictions': predictions_loss},
                             metrics= {'predictions': [smoothness_loss, kl_loss]})
    full_model.fit(valid_dataset, validation_data=valid_dataset, steps_per_epoch=100, epochs=20, validation_steps=1)
    save_predictions(model, valid_data, 'after_ft', use_first_op=False)
